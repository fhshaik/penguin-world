...
switch ($Action) {
 'start' { docker compose up -d db redis houdini_login houdini_blizzard dash economy web }
 'play' {
   ...
   $needsPatch = -not (Test-Path -LiteralPath $patchHashPath)
   if (-not $needsPatch) {
 ...
 'logs' { docker compose logs --tail 60 houdini_login houdini_blizzard economy }
 'backup' {
